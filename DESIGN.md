...
| Class | Labels | cut_t |
|---|---|---|
| stops, affricates | B D G K P T, CH JH | 20% into the phone (inside the closure, before any plausible burst); **exception:** when the preceding phone is SIL, cut_t = phone start — the closure belongs to the silence, keeping the burst intact in the following unit |
...
- `utterances` — span ref, full text, speaker FK, `multi_speaker`, ASR confidence; from WhisperX segments. **FTS5 document unit**: external-content `fts5(text, content='utterances')` kept in sync by the three AFTER INSERT/DELETE/UPDATE triggers, shipped in migration v1. Phrase hits map back to word spans via per-word ordinals.
...
- `speakers` — id, source_id, label, optional human name, `stale` flag. All unit speaker columns are INTEGER FKs here. **Re-ingest carries speakers forward**: a new run's turn-set matching an existing speaker by ≥ 50% temporal overlap of total speech inherits its id (and name); otherwise a new row; orphaned *named* speakers are kept flagged stale, never deleted.
