...
**MFA chunking.** Per WhisperX segment; segments with inter-gap < 300 ms are merged first; each chunk padded 250 ms both sides (clamped to neighbors' midpoints). The sidecar parses each chunk's TextGrid and rebases to master time — **every timestamp in the contract is master-relative**. Phones within 100 ms of a chunk edge get reduced confidence. The resulting chunk spans are part of the manifest contract (`chunks`): the loader needs their edges to insert chunk-origin SIL terminators.
